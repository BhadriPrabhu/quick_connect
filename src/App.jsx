...
function App() {
  ...
  const stopAutomation = (message = null) => {
    chrome.storage.local.set({ isAutoConnecting: false }, () => {
      setIsConnecting(false);
      if (message) {
        setHelper(message);
        console.log(`QuickConnect: ${message}`);
      }
    });
  };
  ...
  const runAutomation = () => {
    ...
    if (isWelcomePage || isAlreadyLoggedIn || isLoginSuccessful) {
    ...
    if (isTryAgain) {
      stopAutomation("Credentials incorrect. Please update in Settings.");
      return;
    }
  ...
  const handleStop = () => {
    stopAutomation("Automation stopped by user.");
  };
